...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your ecomapss-api account.<br>
        ...
        &mdash; ecomapss-api Team
      `
      return sendMail({ toEmail: email, subject: 'ecomapss-api - Password Reset', content })
    })
